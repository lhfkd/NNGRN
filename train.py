...
def main(_):
  ...
  if not os.path.exists(repr(FLAGS.output_name) + '/test'):
    os.mkdir(repr(FLAGS.output_name) + '/test')
  if not os.path.exists(repr(FLAGS.output_name) + '/test/awt'):
    os.mkdir(repr(FLAGS.output_name) + '/test/awt')
  ...
  wt_ = np.genfromtxt('data/axg1.csv', delimiter=',',encoding='utf-8-sig', dtype='float32')
  W1 = tf.Variable(tf.truncated_normal([NUM_GENES+1, 32], stddev=0.1))#
  ...
  x1 = 0.0*tf.ones([1,N1,NUM_GENES])

  for t11 in range(30):
    x1p = x1
    # h10 = tf.concat([x1p,bcd1,tsl1], axis=2)
    h10 = tf.concat([x1p, bcd1], axis=2)
  ...
  # Train
  accuracy_writer = csv.writer(open(repr(FLAGS.output_name)+'/savenet/accuracyaxg.csv', 'w'))
  np.savetxt(repr(FLAGS.output_name)+ '/test/awt/Bcd.csv', bcd1.reshape(-1,1), fmt='%.4g', delimiter=',')
  for i in range(4001):
    # apply grad
    sess.run(train_step)
    ...
    if i%100 == 0:
      ...
      if i%1000 == 0:
        X_test = sess.run(tf.concat([x1,bcd1],axis=2))#,tsl1

        plt.plot(np.linspace(0.05,0.95,N1), np.reshape(X_test,[N1,NUM_GENES+1]))#+2/1
        plt.axis([0,1,0,1.2])

        plt.savefig(repr(FLAGS.output_name)+'/train/axground%d'%i)
        plt.close()
    
    if i%4000 == 3999:
      saver.save(sess, repr(FLAGS.output_name)+'/savenet/axggap-network' , global_step = i)
